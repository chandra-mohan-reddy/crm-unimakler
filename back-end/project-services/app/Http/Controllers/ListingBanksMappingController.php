...
use Validator;
use App\Models\ListingBanksMappingModel;
use Carbon\Carbon;
use App\Helpers\StatusCode;
use Illuminate\Support\Facades\Hash;
use DB;

class ListingBanksMappingController extends Controller
{
    /**
    ...
    */
    ...
    public function index() {
        $data = ListingBanksMappingModel::where( 'mapping_status', '=', 'A' )->get();
    ...
    */
    ...
    public function store( Request $request ) {
        ...
        $storeData = new ListingBanksMappingModel;
        $storeData->project_listing_id  = $request->project_listing_id;
        $storeData->bank_id             = $request->bank_id;
        ...
        if ( $storeData->save() ) {
            $response =  StatusCode::getStatusCodeData(200, 'CREATE', 'Listing Bank Mappings are stored!');
    ...
    */
    ...
    public function show( string $id ) {
        $result = ListingBanksMappingModel::where( 'mapping_status', '=', 'A' )
        ->where( 'id', '=', $id )
        ->first();

        if ( !$result ) {
            $response =  StatusCode::getStatusCodeData(200, 'EMPTY', 'Listing Bank Mappings are not found!');
            return response()->json($response, 200);
        } else {
    ...
    */
    ...
    public function update( Request $request, string $id ) {
        ...
        $lam = ListingBanksMappingModel::find( $id );
        ...
        if ( $lam->save() ) {
            $response =  StatusCode::getStatusCodeData(200, 'CREATE', 'Listing Bank Mappings are updated!');
            $response['data'] = $lam;
            return response()->json($response, 200);
        } else {
            $response =  StatusCode::getStatusCodeData(500);
    ...
    */
    ...
    public function destroy( string $id ) {
        // Delete the specified Listing Bank Mappings
        $lam = ListingBanksMappingModel::find( $id );
        if ( !$lam ) {
            $response =  StatusCode::getStatusCodeData(200, 'EMPTY', 'Listing Bank Mappings are not found!');
            return response()->json($response, 200);
        ...
        if ( $lam->delete() ) {
            $response =  StatusCode::getStatusCodeData(200, 'DELETE', 'Listing Bank Mappings are deleted!');
            return response()->json($response, 200);
        } else {
            $response =  StatusCode::getStatusCodeData(500);
            return response()->json($response, 500);
