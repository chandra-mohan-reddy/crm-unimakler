...
use Validator;
use App\Models\Master\StateModel;
use Carbon\Carbon;
...
class StateController extends Controller
{
    ...
    public function index()
    {
        ...
        $response =  StatusCode::getStatusCodeData(200, 'LIST', 'State');
        $response['data'] = $data;
    ...
    {
        $rules = [
            'state_name'        => 'required|unique:master_states,state_name',
            'state_code'        => 'required|unique:master_states,state_code',
        ...
        $storeData = new StateModel;
        $storeData->state_name                = $request->state_name;
        $storeData->state_code                = $request->state_code;
        $storeData->country_code             = $request->country_code;

        if ($storeData->save()) {
            $response =  StatusCode::getStatusCodeData(200, 'CREATE', 'State stored!');
